...
CMD cd /sycl-blas && \
    ...
    if [ "${SYCL_IMPL}" = 'COMPUTECPP' ]; then \
      if [ "${TARGET}" = 'host' ]; then \
        export COMPUTECPP_TARGET="host" && mkdir -p build && cd build && cmake .. -DBLAS_ENABLE_STATIC_LIBRARY=ON -DGEMM_TALL_SKINNY_SUPPORT=OFF -DSYCL_COMPILER=computecpp -DComputeCpp_DIR=/tmp/ComputeCpp-latest -DCMAKE_PREFIX_PATH=/tmp/OpenBLAS/build -DCMAKE_BUILD_TYPE=Release && make -j$(nproc) && cd test && ctest -VV --timeout 1200; \
      else \
        /tmp/ComputeCpp-latest/bin/computecpp_info && \
        export COMPUTECPP_TARGET="intel:cpu" && mkdir -p build && cd build && cmake .. -DBLAS_ENABLE_STATIC_LIBRARY=ON -DGEMM_TALL_SKINNY_SUPPORT=OFF -DSYCL_COMPILER=computecpp -DComputeCpp_DIR=/tmp/ComputeCpp-latest -DCMAKE_PREFIX_PATH=/tmp/OpenBLAS/build -DCMAKE_BUILD_TYPE=Release && make -j$(nproc) && cd test && ctest -VV --timeout 1200; \
      fi \
    elif [ "${SYCL_IMPL}" = 'DPCPP' ]; then \
      export LD_LIBRARY_PATH="/tmp/dpcpp/lib" && mkdir -p build && cd build && cmake .. -DBLAS_ENABLE_STATIC_LIBRARY=ON -DGEMM_TALL_SKINNY_SUPPORT=OFF -DSYCL_COMPILER=dpcpp -DCMAKE_PREFIX_PATH=/tmp/OpenBLAS/build -DBLAS_ENABLE_CONST_INPUT=OFF -DCMAKE_BUILD_TYPE=Release && make -j$(nproc) && cd test && ctest -VV --timeout 1200; \
    else \
